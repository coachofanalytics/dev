...
for product in LoanProduct.objects.all().order_by('name'):
    print(f'   {product.name:40} | Type: {product.product_type:20} | Active: {product.is_active}')
...
for product in LoanProduct.objects.all():
    ...
    elif 'staff emergency' in name_lower or ('staff' in name_lower and 'emergency' in name_lower):
        new_type = 'staff_emergency'
    elif 'staff development' in name_lower or ('staff' in name_lower and 'development' in name_lower):
...
type_counts = LoanProduct.objects.values('product_type').annotate(
    total=Count('id'),
    active=Count('id', filter=Q(is_active=True))
).order_by('product_type')
for item in type_counts:
    print(f'  - {item["product_type"]:25} : {item["total"]:2} total, {item["active"]:2} active')
