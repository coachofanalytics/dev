...
def dashboard_redirect(request):
    """
    Generic dashboard redirect based on user's category.
    Redirects to the appropriate category-specific dashboard.

    If onboarding is incomplete, redirects to the next onboarding step.
    """
    ...
    try:
        ...
        if not progress.is_complete():
            # Redirect to next onboarding step instead of dashboard
            next_step_url = progress.get_next_step_url()
            if next_step_url:
                return redirect(next_step_url)
            # If no next step URL, proceed to dashboard (shouldn't happen but safe fallback)
    except OnboardingProgress.DoesNotExist:
        # Grandfather existing users - create complete progress
    ...
    # Onboarding complete, proceed to category-specific dashboard
    try:
        profile = request.user.profile
        category_slug = profile.category.slug if profile.category else 'individual'

        # Map category slug to dashboard URL name
        dashboard_map = {
        ...
        dashboard_name = dashboard_map.get(category_slug, 'individual_dashboard')
        return redirect(dashboard_name)
    except Exception:
        # Default to individual dashboard if any error
        return redirect('individual_dashboard')
