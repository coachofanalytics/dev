...
# def call_industry(request):
#     # URL of the FastAPI endpoint
#     fastapi_url = "http://127.0.0.1:8000/industry"

#     # Make the GET request to the FastAPI endpoint
#     response = requests.get(fastapi_url)
#     print('RESPONSE WORKED')
#     # If the FastAPI response is successful, parse the response and send it to the template or as JSON
...
#         # level4 = data.get("level1")
#     else:
#         message = "Error occurred while calling Industry endpoint."

#     # Return the message as JSON or pass to the template
#     print(JsonResponse({"id": id}))
...
def call_industry(request):
    ...
    # If the FastAPI response is successful, parse the response and convert it into the desired format
    if response.status_code == 200:
        data = response.json()  # Parse JSON response
        
        # Convert the data into the same format as CustomerUser.objects.all()
        user_list = [
            {
                'id': user.get('id'),
        ...
        print("Formatted User List:", user_list)

    # Return the formatted data to the template or as JSON
    return render(request, "accounts/api.html", {"data": user_list})
