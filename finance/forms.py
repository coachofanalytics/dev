...
class PaymentForm(forms.Form):
    ...
    amount = forms.DecimalField(
        ...
        decimal_places=2,
        required=True,
        help_text="Enter the amount to be paid",
    )
    currency = forms.ChoiceField(
        choices=[("USD", "USD"), ("EUR", "EUR"), ("KES", "KES")],
        label="Currency",
        required=True,
        help_text="Select the currency for the payment",
    )
    email = forms.EmailField(
        label="Email",
        required=True,
        help_text="Enter the email address for payment confirmation",
    )
    first_name = forms.CharField(
        label="First Name",
        max_length=30,
        required=True,
        help_text="Enter the first name of the payer",
    )
    last_name = forms.CharField(
        label="Last Name",
        max_length=30,
        required=True,
        help_text="Enter the last name of the payer",
    )
    payment_purpose = forms.CharField(
        label="Payment Purpose",
        max_length=100,
        required=True,
        help_text="Enter the purpose of the payment",
    )
    phone = forms.CharField(
        label="Phone Number",
        max_length=15,
        required=False,
        help_text="Enter the phone number for payment confirmation",
    )
