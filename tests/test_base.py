...
class BaseEndpointTester:
    def __init__(self, base_url: str = "http://localhost:8002", endpoint: str = ""):
        ...
        self.logger = setup_logger(f"test_{endpoint}")
        self.results_dir = self.get_results_directory()
    ...
    def log_test_result(self, test_name: str, status: str, details: Dict[str, Any]):
        ...
        Args:
            ...
            status (str): Test status (PASS/FAIL/ERROR)
        ...
        results_file = Path(self.results_dir) / f"{self.endpoint}_results.json"
    ...
    def generate_summary_report(self) -> str:
        ...
        passed_tests = sum(
            1 for r in self.test_results if r["metadata"]["status"] == "PASS"
        )
        failed_tests = sum(
            1 for r in self.test_results if r["metadata"]["status"] == "FAIL"
        )
        error_tests = sum(
            1 for r in self.test_results if r["metadata"]["status"] == "ERROR"
        )
        ...
        for result in self.test_results:
            ...
            if result["validation_details"]["errors"]:
                role_results[role]["validation_errors"].extend(
                    result["validation_details"]["errors"]
                )
            if result["validation_details"]["warnings"]:
                role_results[role]["warnings"].extend(
    ...
    def save_results(self, append: bool = False):
        ...
        if append:
            ...
            existing_files = [
                ...
                if f.endswith(".json")
                and f.startswith(f"test_results_{endpoint_name}_{endpoint_type}")
            ]
            if existing_files:
                ...
                try:
                    ...
                    filename = (
                        f"test_results_{endpoint_name}_{endpoint_type}_{timestamp}.json"
                    )
            else:
                filename = (
                    f"test_results_{endpoint_name}_{endpoint_type}_{timestamp}.json"
    ...
    def validate_response(
        self, response_data: List[Dict[str, Any]], role: str
    ) -> Dict[str, Any]:
