...
from finance.models import (LoanApplication, LoanCollateral, LoanPayment,
                            LoanProduct, Payment)
...
class LoanSystemTester:
        ...
        try:
            ...
            if total > 0:
                ...
                status_breakdown = (
                    ...
                    .annotate(count=Count("id"), total_amount=Sum("loan_amount"))
                    .order_by("-count")
                )

                self.log_success(
                    "Loan Applications", f"Found {total} loan applications"
                )
                ...
                # Calculate totals
                total_requested = applications.aggregate(total=Sum("loan_amount"))[
                    "total"
                ] or Decimal("0")

                approved_total = applications.filter(status="approved").aggregate(
    ...
    def test_loan_by_product(self):
        ...
        try:
            ...
            if applications.exists():
                # Group by product
                product_stats = (
                    applications.values("loan_product__name")
                    .annotate(
                        count=Count("id"),
    ...
    def test_loan_payments_model(self):
        ...
        try:
            ...
            if total > 0:
                self.log_success(
                    "Loan Payments Model", f"Found {total} loan payment records"
                )

                # Calculate amounts
                total_paid = loan_payments.aggregate(total=Sum("amount"))[
                    "total"
                ] or Decimal("0")
        ...
        try:
            ...
            if total > 0:
                ...
                type_breakdown = collateral.values("collateral_type").annotate(
                    count=Count("id"), total_value=Sum("estimated_value")
                )
                ...
                # Total value
                total_value = collateral.aggregate(total=Sum("estimated_value"))[
                    "total"
                ] or Decimal("0")
        ...
        try:
            ...
            if active_loans.exists():
                ...
                for loan in active_loans[:3]:  # Test first 3 loans
                    ...
                    if payments.exists():
                        actual_total = payments.aggregate(total=Sum("amount"))[
                            "total"
                        ] or Decimal("0")
    ...
    def test_data_integrity(self):
        ...
        try:
            ...
            if loans_without_product > 0:
                ...
                self.log_success("Data Integrity", "All loans have product assignments")
            ...
            try:
                ...
                if orphaned_loan_payments > 0:
                    ...
                    self.log_success("Data Integrity", "No orphaned loan payments")
            except:
                pass  # Payment model might not have loan field

            # Check for orphaned payments
            orphaned_payments = Payment.objects.filter(loan__isnull=True).count()

            if orphaned_payments > 0:
                self.log_warning(
                    "Data Integrity", f"{orphaned_payments} orphaned payments"
                )
            else:
                self.log_success("Data Integrity", "No orphaned payments")
