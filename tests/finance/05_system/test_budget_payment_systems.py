...
from finance.models import (Budget, BudgetCategory, BudgetEstimateProjection,
                            BudgetRequest, BudgetSubCategory,
                            MultiYearBudgetPlan, Payment, PaymentMethod,
                            Transaction)
...
class BudgetPaymentTester:
    ...
    def test_budget_models(self):
        ...
        try:
            ...
            self.log_success(
                "Budget Models",
                f"Found {budget_count} budgets ({active_budgets} active)",
            )
        except Exception as e:
    ...
    def test_transaction_integration(self):
        ...
        try:
            ...
            self.log_success(
                "Transaction Amounts", f"Total spending: KES {total_spending:,.2f}"
            )
    ...
    def test_budget_projections(self):
        ...
        try:
            ...
            if total > 0:
                total_projected = projections.aggregate(total=Sum("projected_amount"))[
                    "total"
                ] or Decimal("0")

                avg_confidence = (
                    projections.aggregate(avg=Sum("confidence_score"))["avg"] or 0
                )
                ...
                self.log_error(
                    "Budget Projections",
                    "No projections found - run generate_budget_projections",
                )
        except Exception as e:
    ...
    def test_budget_aggregations(self):
        ...
        try:
            ...
            self.log_success("Budget Aggregations", f"Total budget: KES {total:,.2f}")
        ...
        try:
            ...
            if total > 0:
                ...
                for status in statuses:
                    print(
                        f"   └─ {status['status']}: {status['count']} payments "
    ...
    def test_payment_methods(self):
        ...
        try:
            ...
            if total > 0:
                self.log_success(
                    "Payment Methods", f"Found {total} active payment methods"
                )
    ...
    def test_critical_urls(self):
        ...
        url_tests = [
            ...
            (
                "finance:unified-budget-dashboard",
