...
@login_required
def admin_loan_applications(request):
    """Admin view to manage loan applications - list all submitted applications"""
    # Check if user has admin privileges (you can customize this logic)
    if not request.user.is_superuser and not request.user.is_staff:
        messages.error(request, "Access denied. Admin privileges required.")
        return redirect("finance:loan-home")

    # Get all submitted applications (not draft, not already processed)
    applications = (
...
@login_required
def approve_loan_application(request, pk):
    """Approve a submitted loan application (AJAX-friendly)"""
    if not request.user.is_superuser and not request.user.is_staff:
    ...
    try:
        ...
        if app.status not in ["submitted", "under_review"]:
            return JsonResponse(
                {"success": False, "error": f"Invalid status: {app.status}"}, status=400
            )
        ...
        try:
            from finance.utils import (
                send_loan_approval_notification_to_finance,
                update_loan_approval_metrics,
            )

            send_loan_approval_notification_to_finance(app, request.user)
            update_loan_approval_metrics(app)
        except Exception as e:
        ...
        return JsonResponse(
            {"success": True, "status": app.status, "approved_at": app.approved_at}
        )

    except Exception as e:
        logger.error(f"Error approving loan application {pk}: {e}")
        return JsonResponse(
            {"success": False, "error": "Internal server error"}, status=500
        )
...
@login_required
def reject_loan_application(request, pk):
    """Reject a submitted loan application (AJAX-friendly)"""
    if not request.user.is_superuser and not request.user.is_staff:
        return JsonResponse({"success": False, "error": "Forbidden"}, status=403)
    if request.method != "POST":
        return JsonResponse({"success": False, "error": "POST required"}, status=405)
    ...
    try:
        ...
        # Determine rejection reason and category
        rejection_reason = determine_rejection_reason(app)

        # Update application status
        app.status = "rejected"
        app.rejection_reason = rejection_reason.get("title", "Application rejected")
        app.rejection_category = rejection_reason.get("category", "unknown")
        ...
        # Send notifications
        try:
            from finance.utils import send_rejection_notification_to_finance

            send_rejection_notification_to_finance(
                user=app.borrower,
                rejection_reason=rejection_reason,
                application_data={"id": app.id, "amount": app.amount_requested},
            )
        except Exception as e:
            logger.warning(f"Failed to send rejection notifications: {e}")

        messages.info(request, f"Application {app.application_number} rejected.")
        return JsonResponse(
            {
                "success": True,
                "status": app.status,
        ...
        logger.error(f"Error rejecting loan application {pk}: {e}")
        return JsonResponse(
            {"success": False, "error": "Internal server error"}, status=500
        )
...
@login_required
def loan_analytics(request):
    """Comprehensive loan analytics and management dashboard"""
    from django.db.models import Sum, Avg, Q
    from decimal import Decimal

    # Get all loans for summary calculations
    all_loans = LoanApplication.objects.all()

    # Calculate summary statistics
    summary = {
        "total_applications": all_loans.count(),
        "approved_loans": all_loans.filter(
        ...
        # Financial metrics - avoid computed properties in aggregations
        "total_lent": all_loans.filter(
            status__in=["approved", "active", "repaid"]
        ).aggregate(total=Sum("amount_requested"))["total"]
        or Decimal("0.00"),
        "total_outstanding": Decimal("0.00"),  # Will calculate manually below
        "total_repaid": all_loans.filter(status="repaid").aggregate(
            total=Sum("amount_requested")
        )["total"]
        or Decimal("0.00"),
        "total_interest": all_loans.filter(
            status__in=["approved", "active", "repaid"]
        ).aggregate(total=Sum("total_payable") - Sum("amount_requested"))["total"]
        or Decimal("0.00"),
        # Borrower demographics
        "staff_borrowers": all_loans.filter(
            borrower__category=2, status__in=["approved", "active", "repaid"]
        )
        .values("borrower")
        .distinct()
        .count(),
        "other_borrowers": all_loans.filter(
            ~Q(borrower__category=2), status__in=["approved", "active", "repaid"]
        )
        .values("borrower")
        .distinct()
        .count(),
        "avg_loan_amount": all_loans.filter(
            status__in=["approved", "active", "repaid"]
        ).aggregate(avg=Avg("amount_requested"))["avg"]
        or Decimal("0.00"),
    }
    ...
    for loan in active_loans:
        try:
            ...
            # If there's an error calculating balance, skip this loan
            continue
    ...
    if summary["total_applications"] > 0:
        summary["approval_rate"] = (
            summary["approved_loans"] / summary["total_applications"]
        ) * 100
    else:
        summary["approval_rate"] = 0.0

    # Get loans by status for tabs
    active_loans = all_loans.filter(status="active").select_related(
        "borrower", "loan_product"
    )
    approved_loans = all_loans.filter(
        status__in=["approved", "active", "repaid"]
    ).select_related("borrower", "loan_product")
    under_review_loans = all_loans.filter(status="under_review").select_related(
        "borrower", "loan_product"
    )
    rejected_loans = all_loans.filter(status="rejected").select_related(
        "borrower", "loan_product"
    )
...
@login_required
def guarantor_approval_request(request, loan_id):
    """Display guarantor approval request page"""
    loan = get_object_or_404(LoanApplication, id=loan_id)

    # Check if user is the guarantor
    if request.user != loan.guarantor:
        messages.error(request, "You are not authorized to view this loan application.")
        return redirect("finance:loan-home")

    # Calculate guarantor eligibility score
    from .utils import calculate_guarantor_eligibility_score

    eligibility = calculate_guarantor_eligibility_score(request.user)
    ...
    return render(request, "finance/guarantor/approval_request.html", context)


@login_required
def guarantor_reject_loan(request, loan_id):
    """Process guarantor rejection of loan"""
    if request.method != "POST":
        return JsonResponse({"success": False, "error": "POST required"}, status=405)
    ...
    try:
        # Process rejection
        loan.process_guarantor_approval(approved=False, guarantor_user=request.user)

        messages.info(
            request,
            f"Loan {loan.application_number} rejected. Borrower will be notified.",
        )
        return JsonResponse(
            {
                "success": True,
                "status": loan.status,
                "message": "Loan rejected by guarantor",
            }
        )

    except Exception as e:
        logger.error(f"Error processing guarantor rejection: {e}")
        return JsonResponse(
            {"success": False, "error": f"Error processing rejection: {e}"}, status=500
        )
