"""
Base Service Class for Investing Services

This base class provides common functionality and patterns for all investing services,
...
class BaseInvestingService:
    """
    Base service class for all investing-related services.
    ...
    def _validate_investment_amount(self, amount: Union[int, float, str]) -> float:
        """Validate investment amount with minimum threshold."""
        amount_float = self._validate_amount(amount)
        if amount_float < 100:  # Minimum investment amount
            raise ValidationError("Minimum investment amount is $100")
        return amount_float
    ...
    def _validate_symbol(self, symbol: str) -> str:
        ...
        if len(symbol) < 1 or len(symbol) > 10:
            raise ValidationError("Symbol must be 1-10 characters long")
    ...
    def _log_operation(self, operation: str, user: User, details: Dict[str, Any] = None):
        ...
        self.logger.info(f"Investing service operation: {log_data}")
    ...
    def _handle_error(self, error: Exception, operation: str, user: User = None):
        ...
        self.logger.error(f"Investing service error: {error_data}")
