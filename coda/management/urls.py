...
from management.views import (  # Analytics Dashboard Views
    BackgroundCheckListView, DepartmentUpdateView, DSUListView,
    RequirementDeleteView, RequirementDetailView, RequirementUpdateView,
    TaskCategoryCreateView, TaskDeleteView, TaskDetailView,
    TaskGroupCreateView, TaskListView, TaskUpdateView, UsertaskUpdateView,
    activity_analytics_api, activity_catalog_view, activity_coverage_view,
    activity_forecast_api, activity_forecast_dashboard, activity_summary_api,
    analytics_dashboard, anomaly_detection_api, anomaly_detection_dashboard,
    approve_meeting_link, budget_activity_totals_api,
    budget_evidence_validation_api, budget_forecast_api, compliance_dashboard,
    compliance_history_api, compliance_kpis_api, employee_trend_analysis_api,
    get_intelligent_assignment_suggestions, get_meeting_link_suggestions,
    meeting_link_review_dashboard, override_meeting_link, reject_meeting_link,
    trend_analysis_api, trend_analysis_dashboard)
from management.views_enhanced_dashboard import (enhanced_task_dashboard,
                                                 export_my_data,
                                                 load_more_tasks,
                                                 refresh_dashboard,
                                                 report_issue, request_help,
                                                 submit_evidence,
                                                 task_history_view,
                                                 task_leaderboard,
                                                 tier_analytics)
# Import task reset selective views
from management.views_task_reset_selective import (reset_all_tasks,
                                                   reset_tasks_select)
...
urlpatterns = [
    ...
    path(
        "companyagenda-improved/",
        views.companyagenda_improved,
        name="companyagenda_improved",
    ),
    # userdashboard URL removed - functionality moved to unified dashboard
    path(
        "update-agenda/<str:title>/<int:pk>/",
    ...
    path(
        ...
        views.read_employee_contract,
        name="read_employee_contract",
    ),
    path(
        "confirm_employee_contract/",
        views.confirm_employee_contract,
        name="confirm_employee_contract",
    ),
    ...
    path(
        "verifytaskgroupexists/",
        views.verifytaskgroupexists,
        name="verifytaskgroupexists",
    ),
    ...
    path(
        "clientassessment/",
        views.ClientAssessmentListView.as_view(),
    ...
    path(
        "backgroundchecklist/",
        BackgroundCheckListView.as_view(
            template_name="management/background/backgroundchecklist.html"
        ),
    ...
    path(
        "update_grievance/<int:pk>/",
        views.GrievanceUpdateView.as_view(
            template_name="main/snippets_templates/generalform.html"
        ),
        name="update_grievance",
    ),
    path(
        "update_resolution/<int:pk>/",
    ...
    # DAF v2 and Review URLs
    path("daf/v2/", views.daf_v2_view, name="daf_v2"),
    path("daf/review/", views.daf_review_view, name="daf_review"),
    path(
        "daf/review/comment/<int:task_id>/",
        views.daf_review_comment_view,
        name="daf_review_comment",
    ),
    path(
        "daf/review/ai/<int:task_id>/",
        views.daf_review_ai_generate,
        name="daf_review_ai_generate",
    ),
    path("daf/task/<int:task_id>/approve/", views.approve_task, name="approve_task"),
    path("daf/task/<int:task_id>/reject/", views.reject_task, name="reject_task"),
    path(
        "update_dsu/<int:pk>/",
        views.AssessUpdateView.as_view(
            template_name="main/snippets_templates/generalform.html"
        ),
        name="update_dsu",
    ),
    ...
    path(
        "form_submission_view/", views.form_submission_view, name="form_submission_view"
    ),
    ...
    path(
        "add_justification/",
        views.add_requirement_justification,
    ...
    path(
        "update_advertisement/<int:pk>/",
        views.AdsUpdateView.as_view(),
    ...
    path(
        "api/activity/analytics/", activity_analytics_api, name="activity-analytics-api"
    ),
    # Phase 1: Intelligent Assignment API Endpoints
    path(
        "api/task-assignment/suggestions/",
        get_intelligent_assignment_suggestions,
        name="intelligent-assignment-suggestions",
    ),
    # Phase 1: Meeting Link Review
    path(
        "meeting-links/review/",
        meeting_link_review_dashboard,
        name="meeting-link-review",
    ),
    path(
        "api/meeting-links/<int:link_id>/approve/",
        approve_meeting_link,
        name="approve-meeting-link",
    ),
    path(
        "api/meeting-links/<int:link_id>/override/",
        override_meeting_link,
        name="override-meeting-link",
    ),
    path(
        "api/meeting-links/<int:link_id>/reject/",
        reject_meeting_link,
        name="reject-meeting-link",
    ),
    path(
        "api/meeting-links/<int:link_id>/suggestions/",
        get_meeting_link_suggestions,
        name="meeting-link-suggestions",
    ),
    # Phase 2: Budget Integration API endpoints
    path(
        "api/budget/activity-totals/",
        budget_activity_totals_api,
        name="budget-activity-totals-api",
    ),
    path(
        "api/budget/evidence-validation/",
        budget_evidence_validation_api,
        name="budget-evidence-validation-api",
    ),
    ...
    path(
        "api/trends/employee/<int:employee_id>/",
        employee_trend_analysis_api,
        name="employee-trend-analysis-api",
    ),
    # Phase 3: Compliance KPI API endpoints
    path("api/compliance/kpis/", compliance_kpis_api, name="compliance-kpis-api"),
    path(
        "api/compliance/history/", compliance_history_api, name="compliance-history-api"
    ),
    ...
    path(
        "analytics/forecast/activity/",
        activity_forecast_dashboard,
        name="activity-forecast-dashboard",
    ),
    path(
        "analytics/trends/", trend_analysis_dashboard, name="trend-analysis-dashboard"
    ),
    ...
    path(
        "button-testing/",
        lambda request: render(request, "management/button_testing_dashboard.html"),
        name="button-testing",
    ),
]
