...
from management.utils import (paytime,payinitial,paymentconfigurations,
                               deductions,loan_computation,updateloantable,get_tasks,
                               addloantable,employee_group_level,lap_save_bonus,
...
class UsertaskUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def get_success_url(self):
        task = self.get_object()
        return reverse('management:user_pay') + '?' + urlencode({'username': str(task.employee), 'pay_type': 'usertasks'})
...
def newevidence(request, taskid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if link:
                try:
                    response = requests.head(link, timeout=5)  # Using HEAD for faster validation
                    if response.status_code >= 400:
                        messages.error(request, "The provided link is not valid or accessible.")
                        return render(request, "management/daf/evidence_form.html", {"form": form})
            ...
            thread = threading.Thread(
                ...
                args=(data, temp_file_path, original_filename, request.user, task, GOOGLE_DRIVE_FOLDER_ID)
            )
            thread.start()

            messages.success(request, "Your evidence is being processed. You will be notified upon completion.")
...
def process_evidence_submission(data, temp_file_path, original_filename, user, task, folder_id):
    """
    ...
    try:
        ...
        if temp_file_path:
            ...
            drive_link = upload_file_to_drive(temp_file_path, original_filename, user, task, folder_id)
        ...
        with transaction.atomic():
            ...
            # Update task points
            points, maxpoints = Task.objects.values_list("point", "mxpoint").get(id=task.id)
            if data.get('link') or temp_file_path:
                if data.get('requirement'):
                    ...
                    if points >= maxpoints:
                        maxpoints += 5
                    Task.objects.filter(id=task.id).update(point=points, mxpoint=maxpoints)
        ...
        logger.error(f"Error processing evidence submission for user {user.username}: {e}", exc_info=True)
    finally:
...
def sessions(request,slug="training"):
    if slug=="training":
        ...
        sessions=Training.objects.filter(presenter__category__in=[1, 3, 4, 5, 6, 7],presenter__is_active=True,is_active=True,featured=True,is_mock=False).order_by("-created_date")  # Job_Applicant, Jobsupport, Student, Investor, Vendor, General_User
    elif slug=="mock":
...
def usersession(request, user=None, *args, **kwargs):
    ...
    elif request.user.is_superuser or request.user.category in [1, 3, 4, 5, 6, 7]:  # Job_Applicant, Jobsupport, Student, Investor, Vendor, General_User
        ...
        if form.is_valid():
            ...
            if totalpoints <= 80:
                    skills = {
                            ...
                            "requirements_analysis": form.cleaned_data.get("requirementsAnalysis", 0),
                    ...
                    for skill, score in skills.items():
                        ...
                        if score > 5:
                            ...
                            strong_areas.append({
                                ...
                                "recommendation": suggestions["strong"]["recommendations"].get(skill)
                            })
                        elif score <= 4:
                            ...
                            weak_areas.append({
                                ...
                                "recommendation": suggestions["weak"]["recommendations"].get(skill)
                            })
                            improvements.append(f"Consider improving your {skill.replace('_', ' ').title()} skills.")
                    ...
                    return render(request, "management/departments/hr/course_suggestion.html", context)
            else:
...
def get_next_version_number(service, folder_id, base_name):
    ...
    query = f"'{folder_id}' in parents and trashed=false and name contains '{base_name}_v'"
...
def assignment_upload(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user_folder_id = create_or_get_folder(service, request.user.first_name, parent_id=top_level_folder_id)

            # Create or get the subfolder (assignment type) within the user's folder
            sub_folder_id = create_or_get_folder(service, assignment_type, parent_id=user_folder_id)

            for uploaded_file in files:
                # Get the next version number for this assignment type
                base_name = sanitize_folder_name(assignment_type)
                version_number = get_next_version_number(service, sub_folder_id, base_name)

                # Modify the file name as per the naming convention
                date_prefix = datetime.now().strftime('%Y%m%d')
                original_filename = uploaded_file.name
                file_extension = os.path.splitext(original_filename)[1]
                new_file_name = f"{base_name}_{date_prefix}_v{version_number}{file_extension}"
                ...
                try:
                    ...
                    assignment_content = extract_text_from_file(file_content_stream, file_extension)
                except Exception as e:
                    assignment_content = ''
                    print(f"Error extracting text: {e}")

                # Start a new thread for review generation
                thread = threading.Thread(target=generate_review, args=(assignment.id, assignment_content, assignment_type))
                thread.start()
...
def assignment_detail(request, assignment_id):
    ...
    return render(request, 'management/assignment_detail.html', {
...
def delete_assignment(request, assignment_id):
    ...
    try:
        ...
        messages.error(request, "An error occurred while trying to delete the file from Google Drive.")
    
    # Finally, delete the assignment from the database
    assignment.delete()
    messages.success(request, "Assignment and associated files were deleted successfully.")
...
def get_attendee_duration(request):
    ...
    username = 'Hashim'
    meeting_ids = ['708385093', '994131389', '818660269','825287189','368365165','123530685','816482317']  # Array of meeting IDs (can be dynamic)
                ...
                if attendee.get("name") == username:
                    ...
                    if join_time and leave_time:
                        ...
                        if start_date <= join_time <= today:
                            duration = (leave_time - join_time).total_seconds() / 60  # Duration in minutes
    ...
    if not all_meeting_data:
        context["error"] = "No meetings found in the selected duration for this attendee."
