    ...
    if pay_type in ["payslip", "usertasks", "tasks"]:
        if pay_type == "tasks":
            ...
            tasks = Task.objects.filter(
                employee__is_active=True, employee__is_staff=True
            )
        else:
        ...
        if pay_type == "taskhistory":
            ...
            ).exclude(Q(employee__username__in=excluded_usernames))
        else:
            ...
            filter_conditions |= Q(
                daf_date__month=selected_month, daf_date__year=selected_year
            )
            ...
            if selected_month == last_month.month and selected_year == last_month.year:
                ...
                filter_conditions |= Q(created_at__gte=start_of_current_month)
                # Also include tasks with submission this month
                filter_conditions |= Q(submission__gte=start_of_current_month)
                # Include tasks with NULL daf_date (they might be from last month)
                filter_conditions |= Q(daf_date__isnull=True)
            ...
            if is_current_month:
                ...
                filter_conditions |= Q(
                    daf_date__isnull=True, created_at__gte=start_of_current_month
                )
                filter_conditions |= Q(
                    daf_date__isnull=True, submission__gte=start_of_current_month
                )

            tasks = TaskHistory.objects.filter(employee=employee).filter(
                filter_conditions
            )
            ...
            if tasks.count() == 0:
                ...
                logger.debug(
                    f"Total TaskHistory records for {employee.username}: {total_count}"
                )
    else:
        raise ValueError(
...
def best_employee(task_obj):
    ...
    best_users = tuple(
        sum_of_tasks.filter(sum=max_point).values_list("employee__username")
    )
...
def increment_in_graduation_of_employee(
    employee, max_earning, new_group, PayslipConfig
):
    ...
    if payslipconfig_obj.exists():
        ...
        payslipconfig_admin_obj = PayslipConfig.objects.filter(
            user__username="coda_info"
        )
    ...
    try:
        ...
        def get_finance_task_service():
            class NoOpFinanceService:
                def get_payslip_config(self, employee_id):
                    return None
...
def paytime():
    ...
    return (
        ...
        payday,
        start_day_of_prev_month2,
        last_day_of_prev_month1,
        last_day_of_prev_month2,
        start_day_of_prev_month3,
        last_day_of_prev_month3,
    )
    ...
    if user_data.exists():
        ...
        if active_loan and active_loan.is_outstanding:
            ...
            if payslip_config and hasattr(payslip_config, "loan_repayment_percentage"):
                loan_repayment_percentage = Decimal(
                    str(getattr(payslip_config, "loan_repayment_percentage", 0) or 0)
                )
            ...
            logger.info(
                f"Phase 1 loan computation: amount={loan_amount}, payment={loan_payment}, balance={balance_amount}"
            )
            ...
            if loan_payment > 0:
                ...
                logger.info(
                    f"Payment of {loan_payment} recorded for loan {active_loan.id}"
                )
        ...
        if payslip_config and hasattr(payslip_config, "loan_amount"):
            ...
            loan_payment = (
                round(Decimal(total_pay) * loan_repayment_percentage, 2)
                if loan_repayment_percentage
                else Decimal(0)
            )
            if loan_amount < loan_payment:
                loan_payment = loan_amount
...
def updateloantable(user_data, employee, total_pay, payslip_config):
    ...
    if user_data.exists():
        ...
        if active_loan:
            logger.info(
                f"Updated loan for {employee.username}: balance={active_loan.balance_amount}"
            )
...
def lap_save_bonus(payslip_config):
    ...
    if payslip_config.laptop_status:
        ...
        laptop_bonus = (
            payslip_config.lb_amount
            if payslip_config.user.category == CategoryChoices.APPLICANT
            and payslip_config.user.sub_category
        ...
        total_laptop_savings = (
            Decimal(payslip_config.ls_amount)
            if payslip_config.ls_amount < 20000
...
def deductions(employee, user_data, payslip_config, total_pay):
    ...
    if (
        employee.category == CategoryChoices.APPLICANT
        and employee.sub_category == ApplicantSubCategoryChoices.FULL_TIME
    ):
        if payslip_config:
            ...
            loan_payment = Decimal(
                loan_computation(total_pay, user_data, payslip_config)[1]
            )
    else:
        # print(employee.category, employee.sub_category)
...
def bonus(tasks, total_pay, payslip_config):
    ...
    if payslip_config:
        ...
        # -------------holiday earning-----------
        offpay = (
            payslip_config.holiday_pay
            if month in (12, 1) and day in (24, 25, 26, 31, 1, 2)
            else Decimal(0.00)
        )
        # -------------late Night earning-----------
        # PayslipConfig stores some percentage knobs as floats; keep pay math in Decimal.
        rp_increment_max_percentage = Decimal(
            str(getattr(payslip_config, "rp_increment_max_percentage", 0) or 0)
        )
        rp_increment_percentage = Decimal(
            str(getattr(payslip_config, "rp_increment_percentage", 0) or 0)
        )
        rp_starting_amount = Decimal(
            str(getattr(payslip_config, "rp_starting_amount", 0) or 0)
        )

        latenight_Bonus = round(Decimal(total_pay) * rp_increment_max_percentage, 2)
        # print("latenight_Bonus====>",latenight_Bonus)
        yearly = round(
            rp_starting_amount + (Decimal(total_pay) * rp_increment_percentage), 2
        )
    ...
    sub_bonus = (
        Decimal(bonus_points_ammount)
        + Decimal(latenight_Bonus)
        + +Decimal(EOM)
        + Decimal(EOQ)
        + Decimal(EOY)
        + Decimal(offpay)
...
def get_bonus_and_summary(employee, tasks, total_pay, user_data, payslip_config):
    ...
    bonus_points_ammount, latenight_Bonus, yearly, offpay, EOM, EOQ, EOY, sub_bonus = (
        bonus(tasks, total_pay, payslip_config)
    )
...
def email_template(subject, to, html_content):
    msg = EmailMultiAlternatives(subject, "", settings.EMAIL_HOST_USER, [to])
...
def defined_links(request):
    ...
    if request.user.category == 1:  # Job_Applicant category
        ...
        links.update(
            {
                "My DAF": reverse("management:user_pay")
                + "?"
                + urlencode(
                    {"username": request.user.username, "pay_type": "usertasks"}
                ),
                "Last DAF": reverse("management:user_pay")
                + "?"
                + urlencode(
                    {"username": request.user.username, "pay_type": "usertaskhistory"}
                ),
                "My Evidence": reverse("management:user_evidence")
                + "?"
                + urlencode({"username": request.user.username}),
                "Evidence": reverse("management:user_evidence"),
                "My Sessions": reverse("management:user_session", args=[request.user]),
                "My Time": reverse("accounts:account-profile", args=[request.user]),
            }
        )
    elif request.user.category in [
        1,
        3,
        4,
        5,
            ...
            links.update(
                {
                    ...
                    "New Contract": reverse(
                        "main:display_service", kwargs={"slug": "data_analysis"}
                    ),
                    "Make Payment": reverse("finance:pay"),
                    "Investment Portal": reverse(
                        "investing:user_investments",
            ...
            links.update(
                {
                    ...
                    "My Interview": reverse(
                        "professional_services:question-detail",
                        kwargs={"question_type": "resume"},
                    ),
                    "Job Support": reverse(
                        "professional_services:start_training",
                        kwargs={"slug": "interview"},
                    ),
                    "My Time": reverse("accounts:user-list", args=[request.user]),
                    "My Contract": reverse("finance:mycontract", args=[request.user]),
                    "New Contract": reverse(
    ...
    if request.user.is_superuser:
        ...
        links.update(
            {
                "Tasks": reverse("management:user_pay")
                + "?"
                + urlencode(
                    {
                ...
                "History Tasks": reverse("management:user_pay")
                + "?"
                + urlencode(
                    {
                        "username": "",  # Empty username to represent all users
...
def login_user_in_background(email, password):
    ...
    if user:
        ...
        print(
            f"User {user.email} has been logged in with session ID: {session.session_key}"
        )
...
def upload_file_to_drive(temp_file_path, original_filename, user, task, folder_id):
    ...
    if not GOOGLE_AVAILABLE:
        logger.warning(
            "Google Drive API not available - feature disabled during optimization"
        )
        return None
