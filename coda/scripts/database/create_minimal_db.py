#!/usr/bin/env python3
"""
CODA Minimal Database Setup

This script creates a minimal database setup for local testing
bypassing problematic migrations.
"""
...
def main():
    ...
    print("🗄️ Creating Minimal CODA Database")
    print("=" * 50)
    ...
    try:
        ...
        with connection.cursor() as cursor:
            # Create a simple main_assets table to prevent the error
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS main_assets (
            ...
            # Create a simple main_company table
            cursor.execute("""
        ...
        # Run only essential migrations
        print("📊 Running essential migrations...")
        execute_from_command_line(['manage.py', 'migrate', 'auth', '--settings=coda_project.local_settings'])
        execute_from_command_line(['manage.py', 'migrate', 'contenttypes', '--settings=coda_project.local_settings'])
        execute_from_command_line(['manage.py', 'migrate', 'admin', '--settings=coda_project.local_settings'])
        execute_from_command_line(['manage.py', 'migrate', 'sessions', '--settings=coda_project.local_settings'])
        print("✅ Essential migrations completed")
