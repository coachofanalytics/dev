...
urlpatterns = [
    ...
    re_path(r'^data/(?P<path>.*)$', RedirectView.as_view(url='/professional_services/%(path)s', permanent=True)),
    re_path(r'^get_data/(?P<path>.*)$', RedirectView.as_view(url='/ai_services/%(path)s', permanent=True)),
...
# Conditionally include projectmanagement URLs if available
try:
    import_module('projectmanagement.urls')
    urlpatterns.append(
        path(
            "projectmanagement/",
